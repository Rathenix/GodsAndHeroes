...
public class GhUnitGenerator : MonoBehaviour, IUnitGenerator
{
    ...
    {
        var GhCells = new List<GhSquare>();
        foreach (GhSquare cell in cells)
        {
            GhCells.Add(cell);
        }
        return SpawnUnits(GhCells);
    }

    public List<Unit> SpawnUnits(List<GhSquare> cells)
    {
        ...
        for (int i = 0; i < UnitsParent.childCount; i++)
        {
            ...
            if (unit != null)
            {
                ...
                {
                    cell.IsTaken = true;
                    cell.unitOwner = unit.PlayerNumber;
                    unit.Cell = cell;
